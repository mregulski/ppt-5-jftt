...
vector<Instruction> If::gen_ir(Imp::label *cur_label, Imp::Reg reg) {
    ...
    Instruction jump_else = Instruction::JUMP(Instruction::NoArg, cur_label); // fill in the target later
    out.push_back(jump_else);

    // do_then
    auto branch_then = do_then->gen_ir(cur_label, R1);
    out.insert(out.end(), branch_then.begin(), branch_then.end());
    Instruction jump_end = Instruction::JUMP(Instruction::NoArg, cur_label); // fill in the targte later
...
vector<Instruction> Eq::gen_ir(Imp::label *cur_label, Imp::Reg reg) {
    ...
    // jump MAYBE
    out.push_back(Instruction::JUMP(sub_rl.back().label + 3, cur_label)); // TODO: check for off-by-one
    ...
    // jump END
    out.push_back(Instruction::JUMP(5+*cur_label, cur_label)); // TODO: check if it's not off-by-one

    // ifZero r2 jump TRUE
    out.push_back(Instruction::JZERO(R2, 3+*cur_label, cur_label));

    // jump END
    out.push_back(Instruction::JUMP(3 + *cur_label, cur_label));
